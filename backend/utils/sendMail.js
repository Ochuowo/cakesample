...
const sendMail = async (id, email, option) => {
	...
	if (option === 'email verification') {
		...
		const mailOptions = {
			...
			to: email,
			subject: 'Confirm your email for Cakes.co.ke', // Subject line
			html: `<div>
	...
	// send e-mail for resetting password if user forgot password
		...
		const mailOptions = {
			from: process.env.MAIL_USERNAME, // sender address
			to: email,
			subject: 'Reset Password for Cakes.co.ke', // Subject line
			html: `<div>
					<h2>Reset Password for your Cakes.co.ke account</h2>
					<br/>
