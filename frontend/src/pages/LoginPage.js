...
const LoginPage = ({ location, history }) => {
	...
	useEffect(() => {
		if (location.search.includes('login=failed')) {
			...
			switch (errorCode) {
				...
				case '3':
					setAuthFailedMsg(
						'It looks like you had registered using a Facebook Account'
					);
					break;
				default:
					return;
	...
	useEffect(() => {
		...
		if (flag && flag === 'true') {
			...
			history.push('/login'); // move to login page by default
		} else {
			...
			if (isSuccess) {
				...
				axios
					...
					.then(({ data }) => {
						...
						// remove variable that was meant to prompt email verification if it is a social login
	...
	// to send a mail for resetting password if forgotten
