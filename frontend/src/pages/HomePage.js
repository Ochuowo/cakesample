...
const HomePage = ({ match, history }) => {
	...
	// get the products list, userinfo and user details from the redux store
	...
	// refresh token to get a new access token if there's an error in user details
	useEffect(() => {
	...
	// set a state variable to true or false depending on whether the product is avialable in the state
	useEffect(() => {
		if (products) {
			products.length
	...
	// fetch products from the redux store into a local state
	useEffect(() => {
		if (productList) {
	...
	// list products based on keyword and page number
	useEffect(() => {
		dispatch(listProducts(keyword, pageNumber));
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [dispatch, keyword, pageNumber]);

	// check if user needs to be prompted for email verification on page load
	useEffect(() => {
		setPromptVerification(
			localStorage.getItem('promptEmailVerification') === 'true'
				? true
				: false
		);
	}, []);
	...
	return (
		<>
			...
			{/* if the user needs to be prompted for email verification, show this message */}
			{promptVerification ? (
