			...
			<Table className={classes.table} aria-label='simple table'>
				<TableHead>
					<TableRow>
						...
						{/* custom heads */}
						{props.students && <TableCell align='center'>Lesson</TableCell>}
					...
					{props.items.map(item => {
						return (
							<TableRow key={item._id.toString()}>
								...
								{/* Lessons */}
								{props.students ? item.lesson?.length > 0 ? (
									<TableCell align='center'>{item.lesson[0].name}</TableCell>
								) : (
									// adding student to lesson form
									<TableCell align='center'>
