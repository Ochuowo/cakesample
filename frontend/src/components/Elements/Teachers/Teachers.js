...
export default class Teachers extends Component {
	...
	deleteTeacher = async (teacherId, teacherName) => {
		if (window.confirm(`Are you sure you want to delete a teacher with the name ${teacherName} ?`)) {
			try {
	...
	search = async () => {
		...
		this.setState({ loading: true });
		try {
			const res = await axios.get(`api/teachers/search/${this.state.searchText}`);
			this.setState({ loading: false });
