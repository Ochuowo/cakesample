	...
	getTeacher = async () => {
		...
		try {
			...
			if (res.data.teacher.subject.length < 1) {
				const doneObj = {
					message: 'This Teacher has no subject, you should appoint a subject for him/her',
					type: 'warning'
				};
				...
				{this.state.teacher && (
					<div>
						...
						<ul style={{ fontSize: '1.2rem' }}>
							...
							<li>
								<span style={{ fontWeight: 'bold' }}>Avatar</span>: ${this.state.teacher.avatar}
							</li>
							<li>
								<span style={{ fontWeight: 'bold' }}>IsConfirmed</span>: ${this.state.teacher.isConfirmed}
							</li>
						</ul>
					</div>
				)}
